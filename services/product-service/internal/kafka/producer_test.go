...
func TestProducerPublishWaitsForDelivery(t *testing.T) {
	client := &producerClientStub{deliver: true}
	producer := &Producer{client: client}
	require.NoError(t, producer.Publish(context.Background(), "product.updated", []byte("7"), []byte("payload")))
}
...
func TestProducerPublishReturnsDeliveryError(t *testing.T) {
	...
	err := producer.Publish(context.Background(), "product.updated", nil, []byte("payload"))
...
func TestProducerPublishHonorsContext(t *testing.T) {
	...
	defer cancel()
	require.ErrorIs(t, producer.Publish(ctx, "product.updated", nil, nil), context.DeadlineExceeded)
}
