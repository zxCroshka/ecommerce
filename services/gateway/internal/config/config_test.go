...
func TestLoadConfig(t *testing.T) {
	...
	t.Setenv("APP_ORDER_SERVICE_ADDRESS", "order-service.test:9094")
	t.Setenv("APP_NOTIFICATION_SERVICE_ADDRESS", "notification-service.test:9095")
	...
	require.Equal(t, 8*time.Second, cfg.HTTP.RequestTimeout)
	...
	require.Equal(t, "order-service.test:9094", cfg.OrderService.Address)
	require.Equal(t, "notification-service.test:9095", cfg.NotificationService.Address)
	...
	require.Equal(t, 2*time.Second, cfg.OrderService.Timeout)
	require.Equal(t, 2*time.Second, cfg.NotificationService.Timeout)
}
